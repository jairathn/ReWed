...
export default function TodosPage({
  ...
  const urgent = todos.filter((t) => t.status === 'open' && (t.urgency === 'red' || t.urgency === 'orange'));
  const aging = todos.filter((t) => t.status === 'open' && t.urgency === 'yellow');
  const fresh = todos.filter((t) => t.status === 'open' && t.urgency === 'fresh');
  ...
  return (
    <div style={{ maxWidth: 880 }}>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 16 }}>
        {([
          { id: 'open' as const, label: `Open (${todos.filter((t) => t.status === 'open').length})` },
